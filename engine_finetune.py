...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    results_path = os.path.join(task,task+'_metrics_{}.csv'.format(mode))
    with open(results_path,mode='a',newline='',encoding='utf8') as cfa:
        wf = csv.writer(cfa)
        data2=[[acc,sensitivity,specificity,precision,auc_roc,auc_pr,F1]]
        for i in data2:
            wf.writerow(i)
    ...
    if mode=='test':
        ...
        plt.savefig(os.path.join(task,task+'_confusion_matrix_test.jpg'),dpi=600,bbox_inches ='tight')
